...
function loadEvent() {
  document
    .querySelector("#root")
    .insertAdjacentHTML("beforeend", headerComponent("2022"));
  document
    .querySelector("#root")
  ...
  for (let m = 0; m < year.length; m++) {
    ...
    document
      ...
      .insertAdjacentHTML(
        "beforeend",
        monthComponent(year[m].name, year[m].nth)
      );

    for (let dw = 0; dw < daysOfWeek.length; dw++) {
      document
        .querySelector(`.month-${m + 1} .days-week`)
        .insertAdjacentHTML("beforeend", dayOfWeekComponent(daysOfWeek[dw]));
    }
    for (let pd = 0; pd < date.getDay() - 1; pd++) {
      document
        .querySelector(`.month-${m + 1} .days-container`)
        .insertAdjacentHTML("beforeend", placeholderDayComponent());
    }
    for (let d = 1; d < year[m].days + 1; d++) {
      document
        .querySelector(`.month-${m + 1} .days-container`)
        .insertAdjacentHTML("beforeend", dayComponent(d));
    }
  }
